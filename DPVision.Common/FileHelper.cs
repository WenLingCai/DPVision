...
namespace DPVision.Common
{
    public class FileHelper : IDisposable
    {
        ...
        public static void DelectOutdaysFile(string path,string format,int days)
        {
            try
            {

                DirectoryInfo folder = new DirectoryInfo(path);
                foreach (FileInfo file in folder.GetFiles(format))
                {
                    if (string.Compare(file.Name, System.DateTime.Now.AddDays(-days).ToString("yyyy-MM-dd")) < 0)
                    {
                        file.Delete();
                    }
                }
        ...
        /// </summary>
        /// <param name="path"></param>
        public static void DelectOutdaysDirectory(string path, int days)
        {
                ...
                try
                {
                    ...
                    if (Directory.Exists(path))
                    {
                        ...
                        {
                            if (string.Compare(fold.Name, System.DateTime.Now.AddDays(-days).ToString("yyyy-MM-dd")) < 0)
                            {
                                Directory.Delete(fold.FullName, true);
                            }
                        }
                    }
                }
